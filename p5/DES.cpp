...
void keyShift(uint8_t key[8], int count)
{
	uint8_t space[8];
	uint8_t mask = (count == 1 ? 0x10 : count == 2 ? 0x30 : 0x00);
...
void feistel(const uint8_t* input, const uint8_t* key, uint8_t* result)
{
	...
	uint8_t endi[8] = { 0,0,0,0,0,0,0,0 };
	uint8_t subs[4] = { 0,0,0,0 };
...
void DES_Encrypt(const uint8_t data[8], const uint8_t roundKeys[16][8], uint8_t result[8], int round)
{
	uint32_t feistelResult   = 0;
	uint64_t intermData = 0;

	uint32_t* leftHalf  = (uint32_t*)&intermData;
	uint32_t* rightHalf = (uint32_t*)&intermData + 1;

	permutation(data, DES_IP_BOX, 8, (uint8_t*)&intermData);

	for (int i = 0; i < round; i++)
	{	feistel((uint8_t*)rightHalf, roundKeys[i], (uint8_t*)&feistelResult);

		intermData = *rightHalf & 0xFFFFFFFF | ((uint64_t)((*leftHalf) ^ feistelResult) << 32);

		feistelResult = 0;
	}

	intermData = ((intermData & 0xFFFFFFFF00000000) >> 32) | ((intermData & 0x00000000FFFFFFFF) << 32);
	permutation((uint8_t*)&intermData, DES_IIP_BOX, 8, result);
}
...
void DES_Decrypt(const uint8_t data[8], const uint8_t roundKeys[16][8], uint8_t result[8])
{
	...
	for (int i = 0; i < 16; i++)
	{
		feistel((uint8_t*)rightHalf, roundKeys[15-i], (uint8_t*)&feistelResult);
		intermData = *rightHalf & 0xFFFFFFFF | ((uint64_t)((*leftHalf) ^ feistelResult) << 32);
